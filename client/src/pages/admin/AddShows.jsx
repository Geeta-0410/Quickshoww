        ...
        <li key={date}>
          ...
          <div className="flex flex-wrap gap-2 mt-1 text-sm">
            {dateTimeSelection[date].map((time) => (
              <div
                key={time}
                className="flex items-center gap-2 bg-primary/10 border border-primary/20 text-primary px-3 py-1 rounded-full"
              >
                <span>{time}</span>
                <DeleteIcon
                  onClick={() => handleRemoveTime(date, time)} width={15}
                  className=" ml-2 text-red-500 hover:text-red-700 w-3 h-3 cursor-pointer"
                />
              </div>
            ))}
          </div>
